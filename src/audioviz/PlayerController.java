...
public class PlayerController implements Initializable {
    ...
    private boolean automaticPlay=true;
    @Override
    public void initialize(URL url, ResourceBundle rb) {
        ...
        timeSlider.valueProperty().addListener(new ChangeListener<Number>() {
            ...
            public void changed(ObservableValue<? extends Number> observable,Number oldValue, Number newValue){
                if(mediaPlayer!=null){
                    try{
                        if ((Math.abs(newValue.doubleValue()-oldValue.doubleValue()))>200 ) { //User did drag slider,otherwise ignore, continue automatic play
                            Duration tempDuration=new Duration(newValue.doubleValue());
                            mediaPlayer.seek(tempDuration);
                        }
